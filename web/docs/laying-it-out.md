...
| R-b | One tab per section, or per task? | **Per task: Play, Picture, Shape, Sources, Measure** — and since Stage H, **Effects** between Shape and Sources. | A tab per section would split things used together — moving a generator slider while watching the trigger and the vertical scale. Five tabs, each short enough to fit. The sixth came when the voice's oscillator joined Shape: Shape is what the voice is made of, Effects what is done to the pair after (the plane, the delay and chorus). The trace's Filter then moved from Shape to Picture, when the voice filter arrived: one filter is for looking and the other is the instrument's. |
